...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-white/20 bg-[#1F1209] text-white shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.4)] hover:border-[#F7CE55] hover:text-[#F7CE55] active:scale-95 transition-all duration-300";

export default function SocialCards({ cards, onSelectCard, selectedIndex: externalSelectedIndex }) {
  ...
  useEffect(() => {
    if (externalSelectedIndex !== undefined && externalSelectedIndex !== centerIndex) {
      setCenterIndex(externalSelectedIndex);
    }
  ...
  const cycle = useCallback((direction) => {
    ...
    setCenterIndex(prev => {
      const nextIdx = direction === "right" ? (prev + 1) % totalCards : (prev - 1 + totalCards) % totalCards;
      if (onSelectCard) onSelectCard(nextIdx);
      return nextIdx;
    });
  }, [totalCards, needsPagination, onSelectCard]);
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full min-h-[460px] sm:min-h-[520px]">
          {cards.map((card, index) => {
            ...
            const contentNode = card.renderContent ? (
              card.renderContent(card, isCenter)
            ) : card.imgUrl ? (
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span 
                ...
                onClick={() => {
                  setCenterIndex(i);
                  if (onSelectCard) onSelectCard(i);
                }}
                className={`h-2.5 rounded-full transition-all duration-300 cursor-pointer ${i === centerIndex ? "bg-[#F7CE55] w-8 scale-[1.1]" : "bg-white/20 hover:bg-white/40 w-2.5"}`} 
              />
            ))}
          </div>
